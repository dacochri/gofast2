class ScheduleController < ApplicationController
  ...
  def ScheduleController.schedule_record(id)
    ...
    begin
      current_schedule_record['next_oil_change'] = Maintenance.where("vehicle_id = ? AND maintenance_type = ?", id, 'oil change').select("kilometres").order('kilometres DESC').first.kilometres.to_i + 20000
    rescue
      current_schedule_record['next_oil_change'] = 'No Oil Change Records Found'
    end
    ...
    begin
      ...
      current_schedule_record['trailer_next_service_date'] = 'No Safety Records Found'
    end
